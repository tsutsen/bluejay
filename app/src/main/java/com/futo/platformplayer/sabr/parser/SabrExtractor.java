...
public class SabrExtractor implements Extractor {
    ...
    private ParsableByteArray subtitleSample;
    ...
    /**
     * @param trackType The type of the track. Typically one of the {@link androidx.media3.common.C}
    ...
    private void writeSegmentData(MediaSegmentDataSabrPart part) throws IOException {
        // TODO: not implemented
    ...
    private void initExtractorOutput() throws ParserException {
        ...
        if (tracks.size() == 0) {
            throw ParserException.createForUnsupportedContainerFeature("No valid tracks were found");
        }
            ...
            if (track.hasContentEncryption) {
                ...
                if (!sampleSignalByteRead) {
                    ...
                    if ((scratch.getData()[0] & 0x80) == 0x80) {
                        throw ParserException.createForUnsupportedContainerFeature("Extension bit is set in signal byte");
                    }
    ...
    private void writeSubtitleSampleData(ExtractorInput input, byte[] samplePrefix, int size)
        ...
        if (subtitleSample.capacity() < sizeWithPrefix) {
            ...
            subtitleSample = new ParsableByteArray(Arrays.copyOf(samplePrefix, sizeWithPrefix + size));
        } else {
    ...
    private long scaleTimecodeToUs(long unscaledTimecode) throws ParserException {
        if (timecodeScale == C.TIME_UNSET) {
            throw ParserException.createForUnsupportedContainerFeature("Can't scale timecode prior to timecodeScale being set.");
        }
    ...
    private static final class Track {
        ...
        public void initializeOutput(ExtractorOutput output, int trackId) throws ParserException {
            ...
            switch (codecId) {
                ...
                case CODEC_ID_THEORA:
                    // Unknown until proper init data wiring is defined.
                ...
                case CODEC_ID_OPUS:
                    ...
                    initializationData.add(
                            ByteBuffer.allocate(8).order(ByteOrder.nativeOrder())
                                    .putLong(codecDelayNs).array());
                    initializationData.add(
                            ByteBuffer.allocate(8).order(ByteOrder.nativeOrder())
                                    .putLong(seekPreRollNs).array());
                    break;
                case CODEC_ID_AAC:
                ...
                case CODEC_ID_ACM:
                    ...
                    if (parseMsAcmCodecPrivate(new ParsableByteArray(codecPrivate))) {
                        ...
                        if (pcmEncoding == C.ENCODING_INVALID) {
                            ...
                            Log.w(TAG, "Unsupported PCM bit depth: " + audioBitDepth
                                    + ". Setting mimeType to " + mimeType);
                        }
                    } else {
                        mimeType = MimeTypes.AUDIO_UNKNOWN;
            ...
            final String trackIdStr = Integer.toString(trackId);

            // Base builder with fields common across types.
            ...
            if (MimeTypes.isAudio(mimeType)) {
                ...
                // (Bitrate left as NO_VALUE; set it here if you have it.)
                ...
                if (displayWidth != Format.NO_VALUE && displayHeight != Format.NO_VALUE) {
                    pixelWidthHeightRatio =
                            ((float) (height * displayWidth)) / (width * displayHeight);
                }
                ...
                if (width != Format.NO_VALUE) builder.setWidth(width);
                if (height != Format.NO_VALUE) builder.setHeight(height);
                if (rotationDegrees != Format.NO_VALUE) builder.setRotationDegrees(rotationDegrees);
                if (pixelWidthHeightRatio != Format.NO_VALUE) {
                    builder.setPixelWidthHeightRatio(pixelWidthHeightRatio);
                }
                if (projectionData != null) builder.setProjectionData(projectionData);
                if (stereoMode != Format.NO_VALUE) builder.setStereoMode(stereoMode);
                if (colorInfo != null) builder.setColorInfo(colorInfo);

            } else if (MimeTypes.APPLICATION_SUBRIP.equals(mimeType)) {
                type = C.TRACK_TYPE_TEXT;
                // No extra fields needed for SRT.

            } else if (MimeTypes.TEXT_SSA.equals(mimeType)) {
                type = C.TRACK_TYPE_TEXT;
                // Override init data for SSA/ASS: dialogue format header + private data.
                List<byte[]> ssaInit = new ArrayList<>(2);
                ssaInit.add(SSA_DIALOGUE_FORMAT);
                ssaInit.add(codecPrivate);
                builder.setInitializationData(ssaInit)
                        .setSubsampleOffsetUs(Format.OFFSET_SAMPLE_RELATIVE);

            } else if (MimeTypes.APPLICATION_VOBSUB.equals(mimeType)
                    || MimeTypes.APPLICATION_PGS.equals(mimeType)
                    || MimeTypes.APPLICATION_DVBSUBS.equals(mimeType)) {
                type = C.TRACK_TYPE_TEXT; // Image-based subs still use TEXT track type in extractors.

            } else {
                throw ParserException.createForUnsupportedContainerFeature("Unexpected MIME type.");
            }
        ...
        private static Pair<String, List<byte[]>> parseFourCcPrivate(ParsableByteArray buffer)
            ...
            try {
                ...
                if (compression == FOURCC_COMPRESSION_DIVX) {
                    ...
                    throw ParserException.createForUnsupportedContainerFeature("Failed to find FourCC VC1 initialization data");
                }
            } catch (ArrayIndexOutOfBoundsException e) {
                throw ParserException.createForUnsupportedContainerFeature("Error parsing FourCC private data");
            }
        ...
        private static List<byte[]> parseVorbisCodecPrivate(byte[] codecPrivate)
            ...
            try {
                if (codecPrivate[0] != 0x02) {
                    throw ParserException.createForUnsupportedContainerFeature("Error parsing vorbis codec private");
                }
                int offset = 1;
        ...
        private static boolean parseMsAcmCodecPrivate(ParsableByteArray buffer) throws ParserException {
            try {
                ...
                throw ParserException.createForUnsupportedContainerFeature("Error parsing MS/ACM codec private");
            }
        }
    }
}
