...
import com.tsutsen.platformplayer.api.http.server.ManagedHttpServer
import com.tsutsen.platformplayer.api.http.server.handlers.HttpConstantHandler
import com.tsutsen.platformplayer.api.http.server.handlers.HttpContentUriHandler
import com.tsutsen.platformplayer.api.http.server.handlers.HttpFileHandler
import com.tsutsen.platformplayer.api.http.server.handlers.HttpFunctionHandler
import com.tsutsen.platformplayer.api.http.server.handlers.HttpProxyHandler
...
import com.tsutsen.platformplayer.api.media.models.streams.sources.LocalSubtitleSource
...
import com.tsutsen.platformplayer.api.media.platforms.js.models.sources.JSDashManifestMergingRawSource
...
import com.tsutsen.platformplayer.sabr.CastSupersededException
import com.tsutsen.platformplayer.sabr.SabrBlockedException
import com.tsutsen.platformplayer.sabr.SabrFormat
import com.tsutsen.platformplayer.sabr.SabrFormatSubstitutedException
import com.tsutsen.platformplayer.sabr.SabrReloadRequiredException
import com.tsutsen.platformplayer.sabr.SabrSession
...
import com.tsutsen.platformplayer.awaitCancelConverted
import com.tsutsen.platformplayer.builders.DashBuilder
...
import com.tsutsen.platformplayer.exceptions.UnsupportedCastException
import com.tsutsen.platformplayer.findPreferredAddress
import com.tsutsen.platformplayer.logging.Logger
import com.tsutsen.platformplayer.parsers.HLS
import com.tsutsen.platformplayer.states.StateApp
import com.tsutsen.platformplayer.stores.CastingDeviceInfoStorage
import com.tsutsen.platformplayer.stores.FragmentedStorage
import com.tsutsen.platformplayer.toUrlAddress
...
class StateCasting {
    ...
    @Volatile private var _sabrCastProxy: com.tsutsen.platformplayer.sabr.SabrCastProxy? = null
    ...
    @Synchronized
    private fun installCastProxy(proxy: com.tsutsen.platformplayer.sabr.SabrCastProxy, castId: Int, device: CastingDevice): Boolean {
    ...
    @Synchronized
    private fun retireProxy(proxy: com.tsutsen.platformplayer.sabr.SabrCastProxy) {
    ...
    private var _preparingCastProxy: com.tsutsen.platformplayer.sabr.SabrCastProxy? = null

    @Synchronized
    private fun setPreparingCastProxy(proxy: com.tsutsen.platformplayer.sabr.SabrCastProxy?) {
            ...
            DiscoveryEventHandler(
                ...
                { deviceInfo -> // Updated
                    ...
                    if (handle != null && handle is CastingDevice) {
                        // fcast 0.4.1's raw device is immutable; swap in a fresh handle
                        if (handle === activeDevice) {
                            Logger.v(TAG) { "Address update for the active device is ignored (fcast 0.4.1 SDK limitation)" }
                        } else {
                            val replacement = CastingDevice(_context.createDeviceFromInfo(deviceInfo))
                            devices[deviceInfo.name] = replacement
                            invokeInMainScopeIfRequired {
                                onDeviceChanged.emit(replacement)
                            }
                        }
                    }
                },
        ...
        try {
            ...
            val rsDeviceInfo = RsDeviceInfo(
                ...
                protocol = when (deviceInfo.type) {
                    com.tsutsen.platformplayer.casting.CastProtocolType.CHROMECAST -> ProtocolType.CHROMECAST
                    com.tsutsen.platformplayer.casting.CastProtocolType.FCAST -> ProtocolType.F_CAST
            ...
            invokeInMainScopeIfRequired {
                ...
                when (castConnectionState) {
                    CastConnectionState.CONNECTED -> {
                        ...
                        StateApp.withContext { ctx -> UIDialogs.toast(ctx, "Connected to [${device.name}]") }
                    }
                    CastConnectionState.CONNECTING -> {
                        Logger.i(TAG, "Casting connecting to [${device.name}]")
                        StateApp.withContext { ctx -> UIDialogs.toast(ctx, "Connecting to [${device.name}]...") }
                    }
                    CastConnectionState.DISCONNECTED -> {
                        StateApp.withContext { ctx -> UIDialogs.toast(ctx, "Disconnected from device") }
                    }
                }
    ...
    private suspend fun castUMP(video: IPlatformVideoDetails, source: JSUMPSource, subtitleSource: ISubtitleSource?, resumePosition: Double, speed: Double?, castId: Int, preferredVideoHeight: Int = -1, sabrState: SabrSession.Transferable? = null, onError: ((Throwable) -> Unit)? = null, onLoadingEstimate: ((Int) -> Unit)? = null, onLoading: ((Boolean) -> Unit)? = null) : List<String> {
        ...
        if (ad.isSabrSupported()) {
            val sabrUrl = com.tsutsen.platformplayer.sabr.buildSabrUmpUrl(source, bestVideo, bestAudio);
        ...
        val proxy = com.tsutsen.platformplayer.sabr.SabrCastProxy(session, bestVideo, bestAudio);
        ...
        proxy.onFatalError = { error ->
            _scopeMain.launch {
                if (castId == _castId.get()) {
                    ...
                    UIDialogs.toast(StateApp.instance.contextOrNull, when (error) {
    ...
    companion object {
        // Lazy: the class initializer used to construct the instance eagerly,
        // which crashed when Hilt first touched it during App.onCreate before
        // FragmentedStorage's files dir was initialized.
        @Volatile
        private var _instance: StateCasting? = null

        val instance: StateCasting
            get() =
                _instance ?: synchronized(this) {
                    _instance ?: StateCasting().also { _instance = it }
                }
        ...
        const val DEFAULT_USER_AGENT = "Mozilla/5.0 (Windows NT 10.0; rv:91.0) Gecko/20100101 Firefox/91.0";
        const val SABR_CALL_TIMEOUT_MS = 0L;
        const val SABR_READ_TIMEOUT_MS = 30000L;
    }

    // fcast 0.4.1 exposes no receiver capability query; only fcast receivers speak SABR-UMP natively
    private fun CastingDevice.isSabrSupported(): Boolean = protocolType == CastProtocolType.FCAST
}
