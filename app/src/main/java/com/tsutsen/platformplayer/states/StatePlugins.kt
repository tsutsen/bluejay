...
import com.tsutsen.platformplayer.stores.PluginIconStorage
import com.tsutsen.platformplayer.stores.PluginScriptsDirectory
    ...
    fun loginPlugin(context: Context, id: String, afterLogin: ()->Unit): Boolean {
        ...
        LoginDialog.showLogin(config) { result ->
            ...
            if(result == null)
                return@showLogin;
            try {
                StatePlugins.instance.setPluginAuth(config.id, result as? com.tsutsen.platformplayer.api.media.platforms.js.SourceAuth);
            } catch (e: Throwable) {
